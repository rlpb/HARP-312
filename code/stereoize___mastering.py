# -*- coding: utf-8 -*-
"""Stereoize > Mastering.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vlUY5iT9sS26J1kHB-yzbGejAA1Vy0fy
"""
...
# User parameters
# ============================================================
INPUT_PATH = "/content/audiosr_1.wav"  # @param {"type":"string"}

# ============================================================
# Helpers (quiet, idempotent)
# ============================================================
...
# ============================ CPU fallback shim ============================
# Detect whether CUDA is actually usable WITHOUT triggering CUDA init on CPU-only builds
_CUDA_COMPILED = False
try:
    _CUDA_COMPILED = (getattr(torch.version, "cuda", None) is not None) and hasattr(torch._C, "_cuda_getDeviceCount")
except Exception:
    _CUDA_COMPILED = False
...
# If CUDA is not usable, make the original "GPU required" check pass, and
# redirect any attempt to move tensors/modules to CUDA back to CPU.
_ORIG_CUDA_IS_AVAILABLE = None
if not _USE_CUDA:
    try:
        _ORIG_CUDA_IS_AVAILABLE = torch.cuda.is_available
        torch.cuda.is_available = (lambda: True)  # only to bypass the hard check below
    except Exception:
        pass

    # Make torch.cuda.empty_cache() safe on CPU-only / no-driver setups
    try:
        torch.cuda.empty_cache = (lambda: None)
    except Exception:
        pass

    # Remap .to(...cuda...) and .cuda() to CPU so no CUDA lazy init can happen
    _orig_mod_to = torch.nn.Module.to
    _orig_ten_to = torch.Tensor.to
    ...
    def _patched_tensor_to(self, *args, **kwargs):
        ...
        if "device" in kwargs:
            kwargs["device"] = _map_to_cpu(kwargs["device"])
        return _orig_ten_to(self, *args, **kwargs)
    ...
    def _force_cpu_cuda(self, *args, **kwargs):
        return self.to("cpu")

    try:
        torch.nn.Module.cuda = _force_cpu_cuda
        torch.Tensor.cuda = _force_cpu_cuda
    except Exception:
        pass
# ===============================================================================

if not torch.cuda.is_available():
    raise RuntimeError("GPU is required (CUDA not available).")

# --------------------------- restore real is_available -------------------
if (not _USE_CUDA) and (_ORIG_CUDA_IS_AVAILABLE is not None):
    try:
...
# ============================================================
INPUT_PATH = "/content/audiosr_1.wav"      # @param {"type":"string"}
OUTPUT_PATH = "/content/premaster_1.wav"   # @param {"type":"string"}
...
display(Markdown(r"""
# Stereo synthesis (mono-safe, transient-safe, no-fallback)
...
No-fallback (non-degenerate inputs):
- A valid solution always exists via scale=0 (Side=0 ⇒ stereo collapses to the mono reference).
- Near-silent or corrupted inputs are rejected explicitly.
"""))
...
# User parameters
# ============================================================
INPUT_PATH  = "/content/premaster_1.wav"  # @param {"type":"string"}
OUTPUT_PATH = "/content/master_1.wav"     # @param {"type":"string"}
